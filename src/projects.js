...
export const projects = [
...

  {
    ...
    origin: `My father runs Metrotécnica, a metrology and instrument calibration company in Várzea Paulista. For years, the entire operation ran on a legacy FoxPro system from the early 2000s — a pile of DBF files that only one person knew how to operate, with no backups, no web access, and certificates that were printed and manually signed.\n\nHe asked me if I could "modernize" it. What started as a weekend project quickly became one of the most technically complex things I've ever built. Migrating two decades of calibration history, modeling multi-tenant isolation so that each client company could only see their own instruments, building a WeasyPrint PDF pipeline that matched the exact layout of the original certificates, and adding an electronic signature system with SHA-256 hash and QR code validation — all while keeping the existing workflow intact so my father didn't have to change how he worked.`,

    role: 'Full-Stack Developer (solo)',
    overview: `Metrotécnica is a multi-tenant SaaS built on Flask + SQLAlchemy + PostgreSQL that manages the complete lifecycle of measurement instruments: registration, calibration history, certificate generation, scheduling, and client portal access.\n\nThe system supports multiple client companies (tenants) under a single installation, with full data isolation enforced at the query level. An admin dashboard lets the lab operator impersonate any client, upload legacy DBF ZIP exports for one-click migration, and bulk-upload physical PDF certificates that get matched to calibration records via regex.\n\nCertificates are generated server-side with WeasyPrint, include SHA-256 document hashes, QR codes linking to a public validation URL, and dual electronic signatures (metrologista + responsável). Clients access their instrument list through a React-inspired Jinja2 dashboard with real-time status badges (APROVADO / REPROVADO / VENCIDO).`,

    highlights: [
      'Full FoxPro DBF → PostgreSQL migration: reads .DBF files with latin-1 encoding, maps accented field names, converts day-count frequency to months, and creates tenants atomically',
      'Multi-tenant architecture: every query is scoped by tenant_id extracted from the JWT claim — no raw SQL bypasses the isolation',
      'WeasyPrint PDF pipeline with external CSS, embedded signature images, QR code (qrcode library) and SHA-256 integrity hash',
      'Chunked ZIP upload (5MB parts) for bulk certificate binding — server reassembles, extracts, and regex-matches filenames to calibration records',
      ...
      'CNPJ and CEP auto-fill via BrasilAPI proxy (avoids CORS on the frontend)',
    ],

    lessons: `This project taught me that legacy migration is a product problem before it's a technical one. The FoxPro data was full of inconsistencies — fields with accents that varied by record, dates stored as strings in three different formats, frequency values in days that had to be reverse-engineered into months. Solving that gracefully, without losing any historical record, required treating every row as a negotiation between what the old system intended and what the new system needed. I also learned that "multi-tenant" is not a feature you add — it's an architectural constraint that has to be present in the first migration or you'll be refactoring forever.`,

    team: 'Solo project (client: Metrotécnica)',
    duration: '5 months (ongoing)',
    stack: [
      { label: 'Backend', value: 'Flask + SQLAlchemy + PostgreSQL' },
      { label: 'Frontend', value: 'Vanilla JS + Tailwind CSS (Jinja2 templates)' },
      { label: 'PDF Engine', value: 'WeasyPrint + qrcode + PIL' },
      { label: 'Auth', value: 'Flask-JWT-Extended (httpOnly + query_string)' },
      { label: 'Legacy Migration', value: 'dbfread (latin-1 DBF → PostgreSQL)' },
      { label: 'Deploy', value: 'Vercel (serverless) + Neon PostgreSQL' },
    ...
    origin_pt: `Meu pai é dono da Metrotécnica, empresa de metrologia e calibração de instrumentos em Várzea Paulista. Por anos, toda a operação rodava num sistema legado em FoxPro dos anos 2000 — uma pilha de arquivos DBF que só uma pessoa sabia operar, sem backup, sem acesso web, e com certificados impressos e assinados à mão.\n\nEle me pediu se eu conseguia "modernizar" aquilo. O que começou como um projeto de fim de semana rapidamente se tornou uma das coisas tecnicamente mais complexas que já construí. Migrar duas décadas de histórico de calibração, modelar o isolamento multi-tenant para que cada empresa cliente visse só seus instrumentos, construir um pipeline de PDF com WeasyPrint que respeitasse exatamente o layout dos certificados originais, e adicionar um sistema de assinatura eletrônica com hash SHA-256 e validação por QR code — tudo isso mantendo o fluxo de trabalho existente para que meu pai não precisasse mudar como trabalhava.`,

    role_pt: 'Desenvolvedor Full-Stack (solo)',
    overview_pt: `A Metrotécnica é um SaaS multi-tenant construído em Flask + SQLAlchemy + PostgreSQL que gerencia o ciclo de vida completo de instrumentos de medição: cadastro, histórico de calibrações, geração de certificados, agendamento e acesso do portal do cliente.\n\nO sistema suporta múltiplas empresas clientes (tenants) numa única instalação, com isolamento total de dados aplicado no nível da query. Um painel administrativo permite que o operador do laboratório acesse qualquer cliente, faça upload de exports ZIP com arquivos DBF legados para migração em um clique, e vincule PDFs de certificados físicos aos registros de calibração via regex.\n\nOs certificados são gerados server-side com WeasyPrint, incluem hash SHA-256 do documento, QR codes que apontam para uma URL pública de validação, e duas assinaturas eletrônicas (metrologista + responsável). Os clientes acessam a lista de instrumentos por um dashboard com status em tempo real (APROVADO / REPROVADO / VENCIDO).`,

    highlights_pt: [
      'Migração completa FoxPro DBF → PostgreSQL: lê arquivos .DBF com codificação latin-1, mapeia nomes de campos acentuados, converte frequência em dias para meses, e cria tenants atomicamente',
      'Arquitetura multi-tenant: toda query é filtrada pelo tenant_id extraído do JWT — nenhum SQL bruto burla o isolamento',
      'Pipeline de PDF com WeasyPrint, CSS externo, imagens de assinatura embutidas, QR code (biblioteca qrcode) e hash de integridade SHA-256',
      'Upload ZIP em chunks (partes de 5MB) para vínculo em lote de certificados — o servidor remonta, extrai e faz match por regex nos nomes de arquivo com os registros de calibração',
      ...
      'Auto-preenchimento de CNPJ e CEP via proxy BrasilAPI (evita CORS no frontend)',
    ],

    lessons_pt: `Este projeto me ensinou que migração de legado é um problema de produto antes de ser técnico. Os dados do FoxPro estavam cheios de inconsistências — campos com acentos que variavam por registro, datas armazenadas como string em três formatos diferentes, valores de frequência em dias que precisavam ser revertidos para meses. Resolver isso de forma limpa, sem perder nenhum registro histórico, exigiu tratar cada linha como uma negociação entre o que o sistema antigo pretendia e o que o novo precisava. Aprendi também que "multi-tenant" não é uma feature que se adiciona — é uma restrição arquitetural que precisa estar presente na primeira migração, ou você vai refatorar para sempre.`,
